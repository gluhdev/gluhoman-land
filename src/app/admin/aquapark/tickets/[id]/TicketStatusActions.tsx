...
const FLOW: Array<{ id: string; label: string }> = [
  ...
  { id: 'used', label: 'Використано (увійшов)' },
];

const TERMINAL = ['used', 'cancelled', 'refunded'];
...
export function TicketStatusActions({
  ...
  const update = async (status: string) => {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/admin/aquapark/tickets/${ticketId}/status`, {
        method: 'POST',
  ...
  if (TERMINAL.includes(currentStatus)) {
    return (
      <p className="text-xs text-[#1a3d2e]/55 italic">
        Квиток {currentStatus === 'used' ? 'використано' : currentStatus === 'refunded' ? 'повернено' : 'скасовано'}.
      </p>
    );
  }
